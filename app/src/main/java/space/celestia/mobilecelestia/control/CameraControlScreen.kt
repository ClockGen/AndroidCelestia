...
fun CameraControlScreen(paddingValues: PaddingValues, observerModeTapped: () -> Unit, modifier: Modifier = Modifier) {
    ...
    Column(modifier = modifier
        ...
        StepperRow(name = CelestiaString("Pitch", ""), minusAction = CameraControlAction.Pitch0, plusAction = CameraControlAction.Pitch1, touchDown = {
        ...
        StepperRow(name = CelestiaString("Yaw", ""), minusAction = CameraControlAction.Yaw0, plusAction = CameraControlAction.Yaw1, touchDown = {
            scope.launch(viewModel.executor.asCoroutineDispatcher()) { viewModel.appCore.keyDown(it.value) }
        }, touchUp = {
            scope.launch(viewModel.executor.asCoroutineDispatcher()) { viewModel.appCore.keyUp(it.value) }
        }, modifier = internalViewModifier)
        StepperRow(name = CelestiaString("Roll", ""), minusAction = CameraControlAction.Roll0, plusAction = CameraControlAction.Roll1, touchDown = {
            scope.launch(viewModel.executor.asCoroutineDispatcher()) { viewModel.appCore.keyDown(it.value) }
        }, touchUp = {
            scope.launch(viewModel.executor.asCoroutineDispatcher()) { viewModel.appCore.keyUp(it.value) }
        }, modifier = internalViewModifier)
        ...
        Spacer(modifier = Modifier.height(dimensionResource(id = R.dimen.list_spacing_short)))
        StepperRow(name = CelestiaString("Zoom (Distance)", ""), minusAction = CameraControlAction.ZoomOut, plusAction = CameraControlAction.ZoomIn, touchDown = {
            scope.launch(viewModel.executor.asCoroutineDispatcher()) { viewModel.appCore.keyDown(it.value) }
        }, touchUp = {
            scope.launch(viewModel.executor.asCoroutineDispatcher()) { viewModel.appCore.keyUp(it.value) }
        }, modifier = internalViewModifier)
